...
def test_ai_self_reference_catches_review_bot_summary_headers():
    # Corpus finding (2026-06-11): AI review bots stamp their summary into the
    # PR body under a markdown header - 25+ AI records across 4 tools, 0/1993
    # humans. Anchored to the header form, never free prose.
    for header in ("## Summary by CodeRabbit", "### Summary by Sourcery"):
        d = Document(body=f"Adds retry logic.\n\n{header}\n\n* **Bug Fixes**")
        assert "ai_self_reference" in hits_by_name(d), header


def test_ai_self_reference_catches_coderabbit_auto_comment():
    d = Document(
...
def test_ai_self_reference_catches_aider_auto_generated_pr_header():
    d = Document(body="## 🤖 Auto-Generated PR\n\n### 📝 Summary\n\nAdd toggle")
    assert "ai_self_reference" in hits_by_name(d)


def test_review_bot_names_in_prose_do_not_fire():
    # The bot NAME is not the marker - the stamped header is. Discussing the
    # bots, or a bare Auto-Generated PR header without the robot stamp (the
    # Renovate/template world), stays safe: attribution floors to HIGH, so a
    # prose mention firing would confidently false-accuse (the D-14 rule).
    for body in (
        "I rewrote the summary by CodeRabbit; Sourcery flagged nothing.",
        "## Auto-Generated PR\n\nThis PR was generated by [Mend Renovate].",
        # Naming the bot by domain is how a human asks to turn it off.
        "The release notes by coderabbit.ai keep appearing - how do I disable them?",
        # Not a rendered header without the space (CommonMark ATX rule).
        "#summary by coderabbit was the tag we grepped for",
    ):
        assert "ai_self_reference" not in hits_by_name(Document(body=body)), body
